...
class MLStrategy(BaseStrategy):
    ...
    def train_model(self, df, horizon=3, threshold=0.01, test_size=0.2, save_path=None):
        ...
        # 최소 데이터 수 완화
        min_data_count = max(50, len(X) * 0.8)  # 최소 50개 또는 80%
        ...
        # 훈련/테스트 분할 (시간 순서 유지)
        if len(X) < 100:
            # 데이터가 적으면 test_size 축소
            test_size = min(test_size, 0.1)
        ...
        # 테스트 세트가 너무 작으면 조정
        if len(X_test) < 5:
            split_idx = max(len(X) - 10, int(len(X) * 0.9))
            X_train, X_test = X.iloc[:split_idx], X.iloc[split_idx:]
            y_train, y_test = y.iloc[:split_idx], y.iloc[split_idx:]
        ...
        if self.model_type == 'random_forest':
            # 데이터 크기에 따라 파라미터 조정
            n_estimators = min(30, max(10, len(X_train) // 5))
            max_depth = min(8, max(3, len(X_train) // 10))
    ...
    def generate_signal(self, df, probability_threshold=0.45):
        """매매 신호 생성"""
        # 특성 준비
        features = self.prepare_features(df)
        if features is None or self.model is None:
            # 모델이 없으면 모든 신호를 0으로 설정
            return pd.Series(0, index=df.index)
            
        # 공통 인덱스만 사용
        common_index = features.index.intersection(df.index)
        
        # 예측 결과 담을 시리즈 초기화 (모든 값 0으로)
        ...
        if len(common_index) > 0:
            ...
            signals_subset[buy_probabilities > probability_threshold] = 1  # 매수 신호
            signals_subset[buy_probabilities < (1 - probability_threshold - 0.05)] = -1  # 매도 임계값 조정
